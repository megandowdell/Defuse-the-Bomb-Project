...
def show_menu(screen):
    ...
    print(f"Menu screen size: {WIDTH}x{HEIGHT}")  # Debug screen size
...
def show_about_game_screen(screen):
    WIDTH, HEIGHT = screen.get_size()
    print(f"About Game screen size: {WIDTH}x{HEIGHT}")  # Debug screen size
...
def soldier_select(screen):
    WIDTH, HEIGHT = screen.get_size()
    print(f"Soldier select screen size: {WIDTH}x{HEIGHT}")  # Debug screen size
    ...
    print(f"Is portrait mode: {is_portrait}")  # Debug orientation detection
    ...
    while True:
        ...
        # Portrait mode (RPi) layout
        if is_portrait:
            # Use a 2x2 grid for soldier cards in portrait mode
            cards_per_row = 2
            rows = (len(soldiers) + cards_per_row - 1) // cards_per_row
            
            # Fixed card size based on screen width
            card_width = int(WIDTH * 0.4)
            card_height = int(HEIGHT * 0.22)  # Adjusted to be slightly smaller
            ...
            for i, soldier in enumerate(soldiers):
                ...
                card_y = int(HEIGHT * 0.12) + row * (card_height + int(HEIGHT * 0.04))
            ...
            # Landscape mode layout - using scaled absolute positions
            ...
            total_base_width = (base_card_width * len(soldiers)) + (base_card_spacing * (len(soldiers) - 1))
            base_start_x = (dev_width - total_base_width) // 2
            
            for i, soldier in enumerate(soldiers):
                is_selected = i == selected_index
                
                # Calculate base position
                base_card_x = base_start_x + i * (base_card_width + base_card_spacing)
                base_card_rect = pygame.Rect(base_card_x, base_card_y_pos, base_card_width, base_card_height)
                
                # Scale the card rectangle
                card_rect = scale_rect(base_card_rect, (WIDTH, HEIGHT))
                ...
                # Draw soldier image
                soldier_img = soldier_images[soldier['name']]
                img_x = card_rect.centerx - soldier_img.get_width() // 2
                img_y = card_rect.y + int(card_rect.height * 0.15)
                screen.blit(soldier_img, (img_x, img_y))
                
                # Draw soldier info
                name_text = char_font.render(soldier['name'], True, YELLOW if is_selected else BEIGE)
                name_x = card_rect.centerx - name_text.get_width() // 2
                name_y = card_rect.y + int(card_rect.height * 0.65)
                screen.blit(name_text, (name_x, name_y))
                
                role_text = desc_font.render(soldier['role'], True, BEIGE)
                role_x = card_rect.centerx - role_text.get_width() // 2
                role_y = name_y + name_text.get_height() + 5
        ...
        nav_hint = desc_font.render("Use LEFT/RIGHT to browse, ENTER to select", True, YELLOW)
        nav_hint_x = WIDTH//2 - nav_hint.get_width()//2
        nav_hint_y = desc_box_rect.y + desc_box_rect.height + 8
        screen.blit(nav_hint, (nav_hint_x, nav_hint_y))
        
        # Reference back button position and dimensions
...
def main():
    ...
    # Create screen with selected dimensions    
    screen = pygame.display.set_mode((WIDTH, HEIGHT))
    
    # Verify actual screen dimensions match requested
    ...
    while game_running:
        if game_state == "menu":
            ...
            if soldier == "Menu":
                ...
                # After character selection, we got a Soldier object
                print(f"Selected soldier: {soldier.name}, {soldier.role}")
                game_state = "menu"
...
if __name__ == "__main__":
    # Set to TRUE to test in RPi mode
    # Set to FALSE to test in desktop mode
    os.environ['RPI_MODE'] = 'TRUE'
    
    # Force a graphics buffer reset to ensure proper scaling
